    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, cid,target_cam, target_view) in enumerate(train_loader_stage2):
            ...
            if cfg.MODEL.SIE_VIEW:
                ...
                target_view = None
            with torch.cuda.amp.autocast(enabled=True):
                score, feat, image_features,cls_score_view,view_feature = model(x = img, label = target, cam_label=target_cam, view_label=target_view)
                logits = image_features @ text_features.t()
               # loss = loss_fn(score, feat, target, target_cam, logits)
                loss = loss_fn(score, feat, target, target_cam,cls_score_view,view_feature,logits)
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    model.eval()
                    for n_iter, (img, vid,cid, camid, camids, target_view, _) in enumerate(val_loader):
                        with torch.no_grad():
                            img = img.to(device)
                            if cfg.MODEL.SIE_CAMERA:
                ...
                for n_iter, (img, vid, cid,camid, camids, target_view, _) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        if cfg.MODEL.SIE_VIEW:
                            ...
                            target_view = None
                        feat = model(img, cam_label=camids, view_label=target_view)
                        evaluator.update((feat, vid,cid, camid))
                cmc, mAP, _, _, _, _, _ = evaluator.compute()
                logger.info("Validation Results - Epoch: {}".format(epoch))
                logger.info("mAP: {:.2%}".format(mAP))
                for r in [1, 5, 10]:
...
def id_context_feature(model, train_loader_stage1):
    ...
    with torch.no_grad():
        for n_iter, (img, vid, _,_, _) in enumerate(train_loader_stage1):
            img = img.to(device)
        ...
        #import pdb
        #pdb.set_trace()  
        list_of_imgs=torch.cat(list_of_imgs)      
      #  sorted_ids, orig_index = torch.sort(torch.tensor(list_of_ids), dim = 0)
        sorted_ids, orig_index = torch.sort(torch.cat(list_of_ids), dim = 0)
      #  sorted_ids, orig_index = torch.sort(torch.tensor([item.cpu().detach().numpy() for item in list_of_ids]), dim = 0)
        ...
        for id in range(min_id, max_id + 1):
            ...
            for orig_i in selected_img_orig_indices:
                ...
                with torch.cuda.amp.autocast(enabled=True):
                    ...
                    selected_img_feature = model(selected_img.unsqueeze(0), target, get_image = True)
