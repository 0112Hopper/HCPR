...
def shuffle_unit(features, begin=1, shift=None, group=None):
    """
    features: 输入特征，形状为 [B, N, D]
    begin: 从第几个通道开始重组，默认为 1
    shift: (可选) 每个样本的特征在 N 维度上的移位量，若 None 则动态生成
    group: (可选) 分组数量，用于 Patch Shuffle 操作，若 None 则动态生成
    """
    batchsize, N, dim = features.size()
...
#    if shift is None:
#        # 在 [1, N-1] 中随机采样
#        shift = torch.randint(1, N, (1,)).item()


    shift = torch.randint(1, 8 , (1,)).item()  # [1, 32] 区间
    #shift = torch.randint(1, N//4 + 1, (1,)).item()  # [1, 32] 区间

    #valid_groups = [2, 4, 8, 16, 32, 64, 128]
    valid_groups = [2, 4, 8]
...
#   if group is None:
#        # 获取 N 的因数
#        valid_groups = [g for g in range(2, N+1) if N % g == 0]
#        # 选一个较合理的（偏中间的分组）
#        group = random.choice(valid_groups)
    ...
    try:
        ...
        # 如果不能整除，补齐最后一个组
        pad_size = (group - (N % group)) % group
        if pad_size > 0:
            pad = torch.zeros(batchsize, pad_size, dim, device=features.device)
    ...
    #return x, shift, group  # 返回时把 shift 和 group 也输出，方便追踪
    return x  # 返回时把 shift 和 group 也输出，方便追踪
...
def shuffle_unit_old(features, shift, group, begin=1):
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        block=self.transformer.resblocks[11]
        self.b2 = nn.Sequential(
            copy.deepcopy(block),
            copy.deepcopy(self.ln_post)
        )
    def forward(self, x: torch.Tensor, cv_emb = None):
       ...
       # x = torch.cat([self.view_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        if cv_emb != None:
        #    x = torch.cat([self.view_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
             x[:,0] = x[:,0] + cv_emb
        #import pdb
...
# local feat learning
#        x11 = self.transformer.resblocks[:11](x)
#        x = x11.permute(1, 0, 2)  # LND -> NLD  
#        token = x[:, 0:1]
#       # import pdb
#        #pdb.set_trace()
#        patch_length = x.size(1) // 4
#       # x = shuffle_unit_old(x, 5, 2)
#
#        x = shuffle_unit(x)
#        b1_local_feat = x[:, :patch_length]
#        b1_local_feat = self.b2(torch.cat((token, b1_local_feat), dim=1))
#     #   b1_local_feat = b1_local_feat @ self.proj   
#        local_feat_1 = b1_local_feat[:, 0]
#
#        # lf_2
#        b2_local_feat = x[:, patch_length:patch_length*3]
#        b2_local_feat = self.b2(torch.cat((token, b2_local_feat), dim=1))
#     #   b2_local_feat = b2_local_feat @ self.proj   
#        local_feat_2 = b2_local_feat[:, 0]
#
#        # lf_3
#        b3_local_feat = x[:, patch_length*2:patch_length*3]
#        b3_local_feat = self.b2(torch.cat((token, b3_local_feat), dim=1))
#     #   b3_local_feat = b3_local_feat @ self.proj   
#        local_feat_3 = b3_local_feat[:, 0]
#
#        # lf_4
#        b4_local_feat = x[:, patch_length*3:patch_length*4]
#        b4_local_feat = self.b2(torch.cat((token, b4_local_feat), dim=1))
#    #    b4_local_feat = b4_local_feat @ self.proj   
...
#
#        #return x11, x12, xproj,
#        #return x11, x12, xproj,x12[:,1],[local_feat_1,local_feat_2,local_feat_3,local_feat_4]
#        #return x11, x12, xproj,x12[:,1],[local_feat_1,local_feat_3,local_feat_3,local_feat_4]
#        return x11, x12, xproj,x12[:,1],[local_feat_1,local_feat_2,local_feat_3,local_feat_4]
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int):
    ...
    if vit:
        state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution, w_resolution,2)
    else: #RN50
